...
const { isType, getRange } = require('./util.js');
...
function mockBasicData (define) {
  ...
  if (type === 'string') {
    ...
    const [min = config.string.min, max = config.string.max] = getRange(range);
    ...
    const [min = config.number.min, max = config.number.max] = getRange(range);
